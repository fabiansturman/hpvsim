...
if __name__ == "__main__":
    ...
    multirunner_noprune.calibrate()

    #plot timeline for each calibration
    for c in multirunner_noprune.calibrations:
        c.plot_timeline()
    for c in multirunner_prune.calibrations:
        c.plot_timeline()

    #Make a boxplot of our calibration times
    pruned_durations = multirunner_prune.get_calibration_times()
    notpruned_durations = multirunner_noprune.get_calibration_times()

    print(f"pruned durations= {pruned_durations}")
    print(f"not pruned durations= {notpruned_durations}")

    dic = {'Time': pruned_durations + notpruned_durations,
       'Dataset': ["Nigeria"]*len(pruned_durations+notpruned_durations),
       'Pruning': ['Yes']*len(pruned_durations) + ['No']*len(notpruned_durations)}
    ...
    fig = go.Figure()
    fig.add_trace(pruneCIplot.data[0])
    fig.add_trace(pruneCIplot.data[1])
    fig.add_trace(pruneCIplot.data[2])
    fig.add_trace(nopruneCIplot.data[0])
