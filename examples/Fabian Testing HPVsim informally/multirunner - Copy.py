...
if __name__ == "__main__":
    ...
    name = "C:\\Users\\fabia\\Documents\\GitHub\\hpvsim\\RawTemp\\Aug2724_3"
    seeds = [0,1,2,3,4]
    ...
    datafiles1 =  [ 
                  'Set 1\\cancer_incidence_2000.csv',
                  'Set 1\\cancers_2020.csv',
                  'Set 1\\hpv_prevalence_2000.csv',
                ]
    datafiles2 =  [ 
                  'Set 2\\cancer_incidence_2000.csv',
                  'Set 2\\cancers_2020.csv',
                  'Set 2\\hpv_prevalence_2000.csv',
                ]
    datafiles3 =  [ 
    ...
    shrf2 = op.pruners.SuccessiveHalvingPruner(reduction_factor=2)
    

    #set up multicalibrations
    mc_d1 = make_multical(name = f"{name}_d1",
                          seeds=seeds,
                          n_workers=n_workers,
                          n_agents = n_agents,
                          n_trials = n_trials,
                          ...
                          verbose = verbose)
    mc_d2 = make_multical(name = f"{name}_d2",
                          seeds=seeds,
                          n_workers=n_workers,
                          n_agents = n_agents,
                          n_trials = n_trials,
                          ...
                          verbose = verbose)
    mc_d3 = make_multical(name = f"{name}_d3",
                          seeds=seeds,
                          n_workers=n_workers,
                          n_agents = n_agents,
                          n_trials = n_trials,
                          ...
                          verbose = verbose)
    mc_d4 = make_multical(name = f"{name}_d4",
                          seeds=seeds,
                          n_workers=n_workers,
                          n_agents = n_agents,
                          n_trials = n_trials,
                          ...
                          pruning=pruning,
                          pruner = shrf2,
                          datafiles=datafiles4,
                          verbose = verbose)
    
    mc_d1.calibrations += mc_d2.calibrations
    mc_d3.calibrations += mc_d4.calibrations
    
    #Calibrate each in succession (we calibrate dataset 1 cals with dataset 2 cals, and then dataset 3 cals with dataset 4 cals, to get 10x 1-wroker cals at a time)
    mc_d1.calibrate()
    mc_d3.calibrate()

    #Plot superimposed learning curves
    mc_d1.get_learning_curves(title = f"Learning curves for datasets 1 and 2 (5 cals on each)").show()
    mc_d3.get_learning_curves(title = f"Learning curves for datasets 3 and 4 (5 cals on each)").show()


    #Plot timeline and learning curve for each calibration individually
    i = 0
    for mc in [mc_d1, mc_d3]:
        i+=1
        for c in mc.calibrations:
            c.plot_learning_curve(title=f"Learning curve for dataset {i}")
            c.plot_timeline(title=f"Timeline for dataset {i}")
