...
if __name__ == "__main__":
    ...
    for pruner in pruners:
        #Storing data from the calibrations
        trials_managed = {      #Don't forget to change results (below) if changing this so that the set of keys matches
             #   1:[],
            #   0.5:[],
                0:[]
            }
        results = {         #Don't forget to change trials_managed (above) if changing this so that the set of keys matches
          #      1    :[],   
            # 0.5  :[],
                0    :[]
        }
        ...
        i+=1
        bigname = f"CalibrationRawResults\\a15Aug24_2_p{i}"
