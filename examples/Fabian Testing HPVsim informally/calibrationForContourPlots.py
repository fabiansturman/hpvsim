...
if __name__ == "__main__":#to allow for using several workers in parellell
    ...
    calib_pars = dict(
            beta=[0.00, 0.010, 0.7], #consider changing the upper bound to 0.20, not 0.7
    ...
    calib = hpv.Calibration(
        ...
        datafiles=datafiles,
        total_trials=800, n_workers=10, #have tried so far: 4 trial,10 trial, now trying 800 trial;  #in my bsp i did a 200 trial contour plot, so to binary search what the hihest reasonae resolution is, going for 400-trial
        ...
        pruning = 2, #when pruning, we can suppress warnings with the flag python -W ingore foo.py, when running file foo.py in command prompt.
...
#[('beta', 'hi5_cin_fn_k'), ('hi5_dur_cin_par1','hpv16_cin_fn_k'), ('hpv16_dur_cin_par1', 'hpv18_cin_fn_k'),( 'hpv18_dur_cin_par1', 'hpv_control_prob')]
    ...
    calib.calibrate(die=False,plots=["learning_curve", "timeline", "contour"], detailed_contplot=[('beta','hpv_control_prob')],save_to_csv=None) #bad idea to do a countour plot with many trials - takes a lot of memory
