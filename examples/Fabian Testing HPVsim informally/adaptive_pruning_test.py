...
if __name__ == "__main__":
    ...
    name = "C:\\Users\\fabia\\Documents\\GitHub\\hpvsim\\RawTemp\\Sep2524_4"
    seeds = [11,12,13] #Trying 5 seeds to see if it still takes between 9-10 times as long to run any given one of those cals compared to leaving all 20 workers to that cal, and that these times are pretty consistnet (as this main calling thread should be able to sleep)
    ...
    mc = make_multical(name = f"{name}_hyperband_adaptive",
                          seeds=seeds,
                          n_workers=n_workers,
                          n_agents = n_agents,
                          n_trials = n_trials,
         ...
         #                 leakiness=leakiness,
          #                pruning=1,
           #               pruner = pruners[0],
            #              datafiles=datafiles,
             #             verbose = verbose)
    mc_p1 = make_multical(name = f"{name}_sh3_adaptive",
                          seeds=seeds,
                          n_workers=n_workers,
                          n_agents = n_agents,
                          n_trials = n_trials,
