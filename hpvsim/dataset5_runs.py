...
class MultiCalibration:
    ...
    def worker(self, calibration):
        ...
        with open(f"{self.name}_results.txt", "a") as f:
            print(f"(pmode={pruning_mode}, pruner={pruner}, seed={cal.sim["rand_seed"]}, cal.name={cal.name}): (final loss={final_loss}, duration={cal.elapsed}, trials processed={cal.processed_trials}, failed={cal.failed_trials}, pruned={cal.pruned_trials})", file=f)
...
if __name__ == "__main__":
    ...
    name = "C:\\Users\\fabia\\Documents\\GitHub\\hpvsim\\RawTemp\\09Oct24_1"
    seeds = [10]
    ...
    datafiles =  [ 
                  'Set 5\\cancer_incidence_2000.csv',
                  'Set 5\\cancers_2020.csv',
                  'Set 5\\hpv_prevalence_2020.csv',
                ]
    

    pruners = [op.pruners.HyperbandPruner(min_resource=1, max_resource=n_trials, reduction_factor=3),
               op.pruners.MedianPruner(),
               op.pruners.SuccessiveHalvingPruner(reduction_factor=2),
               op.pruners.SuccessiveHalvingPruner(reduction_factor=3),
               op.pruners.SuccessiveHalvingPruner(reduction_factor=4)
                ] #NOTE THAT THE NUMBER OF PRUNERS I AM USING HERE IS SORTA FIXED IN THE SUCCEEDING CODE, DONT CHANGE THIS
    ...
    mc = make_multical(name = f"{name}_np",
                          ...
                          n_agents = n_agents,
                          n_trials = n_trials,
                          pruning=-1,            #The initial {3} calibrations in this multicalibration are without pruning
                          datafiles=datafiles,
                          verbose = verbose)
    mc_p0 = make_multical(name = f"{name}_p0h",
                          seeds=seeds,
                          n_workers=n_workers,
                          n_agents = n_agents,
                          n_trials = n_trials,
                          ...
                          datafiles=datafiles,
                          verbose = verbose)
    mc_p1 = make_multical(name = f"{name}_p1m",
                          seeds=seeds,
                          n_workers=n_workers,
                          n_agents = n_agents,
                          n_trials = n_trials,
                          ...
                          datafiles=datafiles,
                          verbose = verbose)
    mc_p2 = make_multical(name = f"{name}_p2s2",
                          seeds=seeds,
                          n_workers=n_workers,
                          n_agents = n_agents,
                          n_trials = n_trials,
                          ...
                          datafiles=datafiles,
                          verbose = verbose)
    mc_p3 = make_multical(name = f"{name}_p3s3",
                          seeds=seeds,
                          n_workers=n_workers,
                          n_agents = n_agents,
                          n_trials = n_trials,
                          ...
                          datafiles=datafiles,
                          verbose = verbose)
    mc_p4 = make_multical(name = f"{name}_p4s4",
                          seeds=seeds,
                          n_workers=n_workers,
                          n_agents = n_agents,
                          n_trials = n_trials,
    ...
    mc.calibrations += mc_p3.calibrations
    mc.calibrations += mc_p4.calibrations
    mc.seeds = mc.seeds*6 #this is just done so that there is a 1-2-1 correspondance between seeds in mc.seeds and (the seeds of) each calibration in mc.calibrations, for the plots made by this test rig
